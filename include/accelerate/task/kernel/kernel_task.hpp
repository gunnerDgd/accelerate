...
#include <accelerate/task/kernel/argument_utility.hpp>
...
namespace accelerate::task::kernel {
	...
	public:
		...
		using argument_type      = std::tuple<argument<std::decay_t<KernelArgs>>...>;
	public:
	...
	accelerate::task::kernel::task<KernelReturn(KernelArgs...)>::__M_set_argument(argument_type& args)
{
	...
	auto  arg_size = sizeof(typename std::remove_reference_t<decltype(arg_set)>::argument_type);
